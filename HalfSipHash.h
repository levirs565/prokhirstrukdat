...
/*
    Fungsi hash HalfSipHas
    Disalin dari https://github.com/veorq/SipHash/blob/master/halfsiphash.c
...
*/
int HalfSipHash_Base(const void *in, const size_t inlen, const void *k, uint8_t *out,
                const size_t outlen) {
...
uint64_t HalfSipHash_64(const void* in, const size_t inlen, const void* k) {
    uint64_t result = 0;
    assert(sizeof(result) == 8);
    HalfSipHash_Base(in, inlen, k, (uint8_t*) &result, 8);
    return result;
}