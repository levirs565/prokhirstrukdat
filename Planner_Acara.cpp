...
namespace AddWindow
{
    ...
    UI::TextBox pengunjungTextBox;
    UI::TextBox tanggalTextBox;
    ...
    LRESULT OnAddClick(UI::CallbackParam param)
    {
        event = Event{
            ...
            pengunjungTextBox.getText(),
            Utils::SystemTimeToDateStr(dtpTanggal.GetValue()),
            deskripsiTextBox.getText()};

        event.date.erase(event.date.find_last_not_of(' ') + 1);
        event.date.erase(0, event.date.find_first_not_of(' '));
        ...
        try
        {
            ...
            if (event.visitor.size() == 0)
            {
                throw std::domain_error("Kapasitas Pengunjung Tidak Boleh Kosong");
            }
            for (wchar_t x : event.visitor)
            {
                if (!iswdigit(x))
                {
                    throw std::domain_error("Kapasitas Pengunjung harus berupa angka!");
                }
            }
            if (event.date.size() == 0)
            {
                throw std::domain_error("Tanggal Event Tidak Boleh Kosong");
            }
    ...
    LRESULT OnCreate(UI::CallbackParam param)
    {
        ...
        pengunjung.SetText(L"Kapasitas Pengunjung :");
        ...
        window.controlsLayout = {
            ...
            {UI::ControlCell(UI::SIZE_FILL, UI::SIZE_DEFAULT, &pengunjungTextBox)},
            {UI::ControlCell(UI::SIZE_FILL, UI::SIZE_DEFAULT, &tanggal)},
            {UI::ControlCell(UI::SIZE_FILL, UI::SIZE_DEFAULT, &dtpTanggal)},
            // {UI::ControlCell(UI::SIZE_FILL, UI::SIZE_DEFAULT, &tanggalTextBox)},
...
namespace TabOldEvent
{
    ...
    void DoRefresh()
    {
        ...
        tree.preorder(tree.root, [&](RBNode<Event> *node)
                      {
            if (node->value.date.size() != 4 && ignoreInvalid) return;
            topK.add(&node->value); });
    ...
    LRESULT OnCreate(UI::CallbackParam param)
    {
        ...
        ignoreInvalidYearCheck.SetText(L"Abaikan Event dengan Tahun Tidak Valid");
...
namespace TabDetailsEvents
{
    ...
    void DoRefresh()
    {
...

        {
            ...
            listView.SetText(5, 1, currentEvent.visitor);
            listView.SetText(6, 1, currentEvent.date);
...

        {
            ...
            while (reader.readData())
            {
                Event event{
                    ...
                    Utils::stringviewToWstring(reader.data[visitorIndex]),
                    Utils::stringviewToWstring(reader.data[dateIndex]),
